...
/// specified by the `CODEX_HOME` or `CODEXSTUDY_CODEX_HOME` environment variable.
/// If not set, defaults to `~/.codexStudy`.
///
...
pub fn find_codex_home() -> std::io::Result<AbsolutePathBuf> {
    let codex_home_env = std::env::var("CODEX_HOME")
        ...
        .or_else(|| {
            std::env::var("CODEXSTUDY_CODEX_HOME")
                .ok()
                .filter(|val| !val.is_empty())
        });
