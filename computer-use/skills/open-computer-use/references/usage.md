...
When you need to launch an app or open a file with a specific program, do not switch strategies after one failed guess. Try, in order: `list_apps`, registry App Paths, `Get-StartApps`, `where.exe`, common install directories, then `Start-Process` with the resolved executable. Re-run `list_apps` after launching to confirm the app is running.
