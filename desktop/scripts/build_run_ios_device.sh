...
APP_PATH="src-tauri/gen/apple/build/arm64/CodexStudy.app"
if [[ ! -d "$APP_PATH" ]]; then
  APP_PATH="$(find src-tauri/gen/apple/build -maxdepth 4 \( -name 'CodexStudy.app' -o -name 'Codex.app' \) | head -n 1 || true)"
fi
