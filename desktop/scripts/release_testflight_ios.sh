...
BETA_DESCRIPTION="${BETA_DESCRIPTION:-CodexStudy iOS beta build for external testing.}"
...
REVIEW_NOTES="${REVIEW_NOTES:-CodexStudy iOS beta build for external testing.}"
SKIP_BUILD=0
SKIP_SUBMIT=0
...
Options:
  ...
  --ipa <path>               IPA path (default: src-tauri/gen/apple/build/arm64/CodexStudy.ipa)
...
Examples:
  ...
  ./scripts/release_testflight_ios.sh --skip-build --ipa "src-tauri/gen/apple/build/arm64/CodexStudy.ipa"
USAGE
}
...
if [[ -z "$IPA_PATH" ]]; then
  IPA_PATH="src-tauri/gen/apple/build/arm64/CodexStudy.ipa"
fi

if [[ ! -f "$IPA_PATH" && "$IPA_PATH" == "src-tauri/gen/apple/build/arm64/CodexStudy.ipa" ]]; then
  LEGACY_IPA_PATH="src-tauri/gen/apple/build/arm64/Codex.ipa"
  if [[ -f "$LEGACY_IPA_PATH" ]]; then
    IPA_PATH="$LEGACY_IPA_PATH"
  fi
fi
