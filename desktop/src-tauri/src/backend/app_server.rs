...
use crate::codex_binary::resolve_effective_codex_bin;
...
impl WorkspaceSession {
    pub(crate) fn matches_codex_home(&self, expected: &Option<PathBuf>) -> bool {
        match (&self.codex_home, expected) {
            (None, None) => true,
...
pub(crate) fn build_codex_path_env(codex_bin: Option<&str>) -> Option<String> {
    let effective_codex_bin = resolve_effective_codex_bin(codex_bin);
    ...
    if let Some(bin_path) = effective_codex_bin.as_deref() {
...
pub(crate) async fn check_codex_installation(
    ...
    let output = match timeout(Duration::from_secs(5), command.output()).await {
        Ok(result) => result.map_err(|e| {
            if e.kind() == ErrorKind::NotFound {
                "CodexStudy CLI not found. Reinstall CodexStudy or ensure `codexstudy` (or legacy `codex`) is on your PATH.".to_string()
            } else {
                e.to_string()
            }
        })?,
        Err(_) => {
            return Err(
                "Timed out while checking Codex CLI. Make sure the bundled Codex binary or `codex --version` runs in Terminal."
                    .to_string(),
            );
        }
    };
    ...
    if !output.status.success() {
        ...
        if detail.is_empty() {
            return Err(
                "Codex CLI failed to start. Try running the bundled Codex binary or `codex --version` in Terminal.".to_string(),
            );
        }
        return Err(format!(
            "Codex CLI failed to start: {detail}. Try running the bundled Codex binary or `codex --version` in Terminal."
        ));
    }
...
pub(crate) async fn spawn_workspace_session<E: EventSink>(
    ...
    let codex_home = crate::codex::home::resolve_spawn_codex_home(codex_home)?;
    command.env("CODEX_HOME", codex_home.as_os_str());
    command.env(
        crate::codex::home::CODEXSTUDY_HOME_ENV_VAR,
        codex_home.as_os_str(),
    );
    ...
    let init_response = match init_result {
        ...
        Err(_) => {
            ...
            return Err(
                "Codex app-server did not respond to initialize. Check that the bundled Codex binary or `codex app-server` works in Terminal."
                    .to_string(),
            );
        }
    };
