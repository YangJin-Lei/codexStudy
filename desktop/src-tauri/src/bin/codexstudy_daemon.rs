...
#[path = "../storage.rs"]
mod storage;
#[path = "codexstudy_daemon/transport.rs"]
mod transport;
#[allow(dead_code)]
#[path = "../types.rs"]
mod types;
...
const DAEMON_NAME: &str = "codexstudy-daemon";
const DAEMON_TOKEN_ENV_VAR: &str = "CODEXSTUDY_DAEMON_TOKEN";
const LEGACY_DAEMON_TOKEN_ENV_VAR: &str = "CODEX_MONITOR_DAEMON_TOKEN";
...
fn default_data_dir() -> PathBuf {
    if let Ok(xdg) = env::var("XDG_DATA_HOME") {
        ...
        if !trimmed.is_empty() {
            return PathBuf::from(trimmed).join("codexstudy-daemon");
        }
    }
    ...
    PathBuf::from(home)
        .join(".local")
        .join("share")
        .join("codexstudy-daemon")
}

fn usage() -> String {
    format!(
        "\
USAGE:\n  codexstudy-daemon [--listen <addr>] [--data-dir <path>] [--token <token> | --insecure-no-auth]\n\n\
...
fn parse_args() -> Result<DaemonConfig, String> {
    ...
    let mut token = env::var(DAEMON_TOKEN_ENV_VAR)
        .ok()
        .or_else(|| env::var(LEGACY_DAEMON_TOKEN_ENV_VAR).ok())
    ...
    if token.is_none() && !insecure_no_auth {
        return Err(
            "Missing --token (or set CODEXSTUDY_DAEMON_TOKEN). Use --insecure-no-auth for local dev only."
                .to_string(),
        );
    }
...
mod tests {
    ...
    fn test_state(data_dir: &std::path::Path) -> DaemonState {
        ...
        DaemonState {
            ...
            codex_login_cancels: Mutex::new(HashMap::new()),
            daemon_binary_path: Some("/tmp/codexstudy-daemon".to_string()),
        }
    }
...
fn main() {
    ...
    if let Err(err) = codex_home::configure_process_codex_home(&config.data_dir) {
        eprintln!("{err}");
        std::process::exit(2);
    }
    ...
    runtime.block_on(async move {
        ...
        eprintln!(
            "codexstudy-daemon listening on {} (data dir: {})",
            config.listen,
            state
                .storage_path
                .parent()
