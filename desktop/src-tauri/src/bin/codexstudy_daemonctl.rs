...
const EXPECTED_DAEMON_NAME: &str = "codexstudy-daemon";
const LEGACY_DAEMON_NAME: &str = "codex-monitor-daemon";
...
const APP_IDENTIFIER: &str = "com.codexstudy.desktop";
...
fn resolve_token(token_arg: Option<&str>, settings: Option<&AppSettings>) -> Option<String> {
    trim_non_empty(token_arg)
        .or_else(|| trim_non_empty(env::var(DAEMON_TOKEN_ENV_VAR).ok().as_deref()))
        .or_else(|| trim_non_empty(env::var(LEGACY_DAEMON_TOKEN_ENV_VAR).ok().as_deref()))
        .or_else(|| {
...
fn is_managed_daemon(info: &DaemonInfo) -> bool {
    matches!(
        info.name.as_str(),
        EXPECTED_DAEMON_NAME | LEGACY_DAEMON_NAME
    )
}
...
LISTEN 0      4096   0.0.0.0:4732      0.0.0.0:*    users:(("codexstudy-daem",pid=12345,fd=7))
"#;
...
tcp        0      0 0.0.0.0:4732            0.0.0.0:*               LISTEN      6789/codexstudy-daem
"#;
