...
pub(crate) fn daemon_binary_candidates() -> &'static [&'static str] {
    if cfg!(windows) {
        &[
            "codexstudy_daemon.exe",
            "codexstudy-daemon.exe",
...
pub(crate) fn resolve_daemon_binary_path() -> Result<PathBuf, String> {
    ...
    for env_var in ["CODEXSTUDY_DAEMON_PATH", "CODEX_MONITOR_DAEMON_PATH"] {
        if let Ok(explicit_raw) = std::env::var(env_var) {
...
mod tests {
    ...
    #[test]
    fn daemon_binary_candidates_prioritize_codexstudy_daemon() {
        assert_eq!(daemon_binary_candidates()[0], "codexstudy_daemon.exe");
    }
}
