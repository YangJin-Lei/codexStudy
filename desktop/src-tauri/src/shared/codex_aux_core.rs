...
pub(crate) async fn codex_doctor_core(
    ...
    let resolved_args = codex_args
        ...
        .filter(|value| !value.trim().is_empty())
        .or(default_args);
    let path_env = build_codex_path_env(effective.as_deref());
    let version = check_codex_installation(effective.clone()).await?;
    ...
    let details = if app_server_ok {
        ...
        Some("Failed to run the bundled Codex app-server or `codex app-server --help`.".to_string())
    };
    Ok(json!({
