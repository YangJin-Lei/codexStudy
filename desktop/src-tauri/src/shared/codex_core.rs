...
fn temp_converted_image_path(path: &str, extension: &str) -> PathBuf {
    ...
    let ts = SystemTime::now()
        ...
        .unwrap_or_default();
    std::env::temp_dir().join(format!("codexstudy-image-{safe_stem}-{ts}.{extension}"))
}
...
fn resolve_attachment_path(raw: &str, workspace_path: &str) -> Result<PathBuf, String> {
    ...
    Ok(if path.is_absolute() {
        ...
        PathBuf::from(workspace_path).join(path)
    })
}

fn read_text_attachment(path: &Path) -> Result<(String, bool), String> {
    let metadata = std::fs::metadata(path)
        .map_err(|err| format!("Failed to read attachment metadata: {err}"))?;
    if !metadata.is_file() {
        return Err("Attachment is not a file.".to_string());
    }
    let bytes = std::fs::read(path).map_err(|err| format!("Failed to read attachment: {err}"))?;
    let truncated = bytes.len() > MAX_INLINE_TEXT_ATTACHMENT_BYTES as usize;
    let content_bytes = if truncated {
        &bytes[..MAX_INLINE_TEXT_ATTACHMENT_BYTES as usize]
    } else {
        &bytes
    };
    let content = String::from_utf8(content_bytes.to_vec()).map_err(|_| {
        "Attachment is not valid UTF-8 text. Currently file attachments support text/code/log/config files."
            .to_string()
    })?;
    Ok((content, truncated))
}

fn maybe_apply_file_attachments(
    text: String,
    ...
    for raw_file in files {
        let resolved = resolve_attachment_path(&raw_file, workspace_path)?;
        let display_path = resolved.display().to_string();
        if !seen_paths.insert(display_path.clone()) {
            continue;
        }
        let (content, truncated) = read_text_attachment(&resolved)
            .map_err(|err| format!("Could not read attached file `{display_path}`: {err}"))?;
        let note = if truncated { "\n[truncated]" } else { "" };
        sections.push(format!(
            "<attached_file path=\"{display_path}\">{note}\n{content}\n</attached_file>"
        ));
    }
    ...
    let file_context = format!("Attached file contents:\n\n{}", sections.join("\n\n"));
    if trimmed.is_empty() {
        return Ok(file_context);
    }
    Ok(format!("{trimmed}\n\n{file_context}"))
}
...
async fn maybe_apply_vision_fallback(
    app_settings: &Mutex<AppSettings>,
    model: Option<&str>,
    text: String,
    images: Option<Vec<String>>,
) -> Result<(String, Option<Vec<String>>), String> {
    let Some(images) = images.filter(|images| images.iter().any(|image| !image.trim().is_empty()))
    else {
        return Ok((text, None));
    };

    let settings = app_settings.lock().await.clone();
    let Some(compat) = settings.model_provider_compat.as_ref() else {
        return Ok((text, Some(images)));
    };
    if model.is_none() && !matches!(compat.kind, crate::types::ModelProviderCompatKind::DeepSeek) {
        return Ok((text, Some(images)));
    }
    if provider_compat_bridge::compat_supports_image_input(compat, model) {
        return Ok((text, Some(images)));
    }

    let Some(vision_fallback) = settings.vision_fallback.as_ref() else {
        return Err(format!(
            "The current {} model cannot accept images, and no vision fallback is configured. Open Settings > Codex > Vision fallback or remove the image.",
            provider_compat_bridge::compat_kind_label(compat.kind.clone())
        ));
    };
    if !vision_fallback_core::vision_fallback_is_ready(Some(vision_fallback)) {
        return Err(
            "The current model cannot accept images, and the vision fallback is not fully configured yet. Open Settings > Codex > Vision fallback."
                .to_string(),
        );
    }

    let analysis =
        vision_fallback_core::analyze_images_with_fallback(vision_fallback, &text, &images).await?;
    Ok((
        vision_fallback_core::merge_vision_analysis_into_text(&text, &analysis, vision_fallback),
        None,
    ))
}
...
pub(crate) async fn send_user_message_core(
    ...
    let text = maybe_apply_file_attachments(text, files, &workspace_path)?;
...
pub(crate) async fn turn_steer_core(
    ...
    let (text, images) = maybe_apply_vision_fallback(app_settings, None, text, images).await?;
    let text = maybe_apply_file_attachments(text, files, &workspace_path)?;
    let input = build_turn_input_items(text, images, app_mentions)?;
    let params = json!({
        "threadId": thread_id,
