...
use crate::codex_binary::{is_bundled_codex_bin, resolve_effective_codex_bin};
...
pub(crate) async fn codex_update_core(
    ...
    let resolved = codex_bin
        ...
        .or(default_bin);
    let effective = resolve_effective_codex_bin(resolved.as_deref());
    ...
    let before_version = check_codex_installation(effective.clone())
        .await
        .ok()
        .flatten();

    if is_bundled_codex_bin(resolved.as_deref()) {
        let result = CodexUpdateResult {
            ok: true,
            method: "bundled".to_string(),
            package: Some("CodexStudy".to_string()),
            before_version: before_version.clone(),
            after_version: before_version,
            upgraded: false,
            output: None,
            details: Some(
                "Bundled Codex is updated together with CodexStudy app updates.".to_string(),
            ),
        };
    ...
    let after_version = if method == "unknown" {
        ...
        match check_codex_installation(effective.clone()).await {
            Ok(version) => version,
            Err(err) => {
                let result = CodexUpdateResult {
                    ok: false,
