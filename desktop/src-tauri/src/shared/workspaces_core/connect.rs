...
pub(crate) fn workspace_session_spawn_lock() -> &'static Mutex<()> {
    ...
    Fut: Future<Output = Result<Arc<WorkspaceSession>, String>>,
{
    ...
    if let Some(existing_for_entry) = {
        ...
        if session_process_is_alive(&existing_for_entry).await
            && existing_for_entry.matches_codex_home(&codex_home)
        {
            return Ok(());
        }
    ...
    if let Some(existing_session) = take_live_shared_session(sessions).await {
        if existing_session.matches_codex_home(&codex_home) {
            existing_session
        ...
        kill_session_by_id(sessions, &existing_session.owner_workspace_id).await;
    }
...
mod tests {
    ...
    fn make_session(_entry: WorkspaceEntry, codex_home: Option<PathBuf>) -> Arc<WorkspaceSession> {
    ...
    fn connect_workspace_is_noop_when_already_connected() {
        tokio::runtime::Runtime::new().unwrap().block_on(async {
            ...
            let sessions = Mutex::new(HashMap::from([(
                entry.id.clone(),
                make_session(entry.clone(), codex_home.clone()),
            )]));
    ...
    fn connect_workspace_spawns_when_not_connected() {
        tokio::runtime::Runtime::new().unwrap().block_on(async {
            ...
            connect_workspace_core(
                ...
                &sessions,
                &app_settings,
                move |_entry, _default_bin, _codex_args, codex_home| {
                    let spawn_calls_ref = spawn_calls_ref.clone();
                    let entry_for_spawn = entry_for_spawn.clone();
                    async move {
                        spawn_calls_ref.fetch_add(1, Ordering::SeqCst);
                        Ok(make_session(entry_for_spawn, codex_home))
                    }
                },
            )
            .await
