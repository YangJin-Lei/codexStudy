...
#[cfg(target_os = "macos")]
const TRAY_ID: &str = "codexstudy-tray";
#[cfg(target_os = "macos")]
    ...
    fn build_usage_menu_labels_include_current_usage_section() {
        assert_eq!(
            build_usage_menu_labels(Some(&TraySessionUsage {
                session_label: "12% used 路 Resets 2 hours".into(),
                weekly_label: Some("67% used 路 Resets in 2 days".into()),
            })),
            (
                "Current Usage".into(),
                "Session: 12% used 路 Resets 2 hours".into(),
                Some("Weekly: 67% used 路 Resets in 2 days".into()),
            )
        );
        assert_eq!(
