...
import { CodexHomeMigrationPrompt } from "@app/components/CodexHomeMigrationPrompt";
import { useCodexHomeMigration } from "@app/hooks/useCodexHomeMigration";
import { subscribeCodexNewFocusThread, subscribeTrayOpenThread } from "@services/events";
...
export default function MainApp() {
  ...
  const updaterEnabled =
    !isMobileRuntime && import.meta.env.VITE_ENABLE_APP_UPDATES === "1";
  ...
  const modelDisplayName =
    selectedModel?.displayName?.trim() || selectedModel?.model?.trim() || null;
  ...
  const codexHomeMigrationIsChinese =
    appSettings.uiLanguage === "zh-CN" ||
    ((appSettings.uiLanguage === "system" || appSettings.uiLanguage == null) &&
      typeof navigator !== "undefined" &&
      navigator.language.toLowerCase().startsWith("zh"));
  const codexHomeMigration = useCodexHomeMigration({
    onImported: () => {
      void listThreadsForWorkspaces(workspaces);
    },
  });
  ...
  const computerUseControl = useComputerUseControl({
    workspaces,
    addWorkspaceFromPath,
    connectWorkspace,
    onAddAgent: handleAddAgent,
    onOpenSettings: () => modalActions.openSettings("computer-use"),
  });
  ...
  const visionFallbackReady = Boolean(
    appSettings.visionFallback?.enabled &&
      appSettings.visionFallback.apiKey?.trim() &&
      appSettings.visionFallback.model?.trim(),
  );
  ...
  return (
    <I18nProvider language={appSettings.uiLanguage}>
      <MainAppShell {...mainAppShellProps} />
      {codexHomeMigration.visible &&
      codexHomeMigration.status &&
      codexHomeMigration.status.legacyHomes.length > 0 ? (
        <CodexHomeMigrationPrompt
          isChinese={codexHomeMigrationIsChinese}
          codexHome={codexHomeMigration.status.codexHome}
          legacyHomes={codexHomeMigration.status.legacyHomes}
          busy={codexHomeMigration.busy}
          error={codexHomeMigration.error}
          onImport={(sourcePath) => {
            void codexHomeMigration.importLegacy(sourcePath);
          }}
          onStartFresh={() => {
            void codexHomeMigration.skip();
          }}
          onLater={() => codexHomeMigration.dismiss()}
        />
      ) : null}
    </I18nProvider>
  );
