...
import { SidebarComputerUseSection } from "./SidebarComputerUseSection";
...
const COLLAPSED_GROUPS_STORAGE_KEY = "codexstudy.collapsedGroups";
...
type SidebarProps = {
  ...
  computerUseStatusLoading?: boolean;
  computerUseStarting?: boolean;
  computerUseActionError?: string | null;
  onStartComputerUseSession?: () => void;
...
export const Sidebar = memo(function Sidebar({
  ...
  onWorkspaceDrop,
  computerUseWorkspaceId = null,
  computerUseEnabled = false,
  computerUseReady = false,
  computerUseStatusLoading = false,
  computerUseStarting = false,
  computerUseActionError = null,
  onStartComputerUseSession,
  ...
  const bucketLabels = useMemo<Record<ThreadBucket["id"], string>>(
    () => ({
      now: t("sidebar.bucket.now", "Now"),
      today: t("sidebar.bucket.today", "Earlier today"),
      yesterday: t("sidebar.bucket.yesterday", "Yesterday"),
      week: t("sidebar.bucket.week", "This week"),
      older: t("sidebar.bucket.older", "Older"),
    }),
    [t],
  );
  ...
  const isHiddenComputerUseWorkspace = useCallback(
    (workspace: WorkspaceInfo) => isComputerUseWorkspace(workspace, computerUseWorkspaceId),
  ...
  const showAccountSwitcher =
    isAccountLoginUiEnabled() && Boolean(activeWorkspaceId);
  ...
  const pinnedThreadRows = useMemo(() => {
    ...
    workspaces.forEach((workspace) => {
      if (isHiddenComputerUseWorkspace(workspace)) {
        return;
      }
      if (
        isSearchActive &&
      ...
      groupedWorkspaces
        .map((group) => ({
          ...
          workspaces: group.workspaces.filter(
            (workspace) =>
              !isHiddenComputerUseWorkspace(workspace) &&
              (!isSearchActive ||
    ...
    >
      ...
      >
        <div className="workspace-list">
          ...
          {onStartComputerUseSession && onOpenComputerUseSettings ? (
            <SidebarComputerUseSection
              workspaceId={computerUseWorkspaceId}
              enabled={computerUseEnabled}
              ready={computerUseReady}
              statusLoading={computerUseStatusLoading}
              starting={computerUseStarting}
              actionError={computerUseActionError}
              ...
              isThreadPinned={isThreadPinned}
              onStartSession={onStartComputerUseSession}
          ...
          {!groupedWorkspacesForRender.length && (
            <div className="empty">
              {isSearchActive
                ? t("sidebar.empty.search", "No conversations match your search.")
                : t("sidebar.empty.addWorkspace", "Add a workspace to start.")}
            </div>
          )}
          {isThreadsOnlyMode &&
            groupedWorkspacesForRender.length > 0 &&
            ...
            pinnedThreadRows.length === 0 && (
              <div className="empty">{t("sidebar.empty.noConversations", "No conversations yet.")}</div>
            )}
        </div>
      </div>
      <SidebarBottomRail
        ...
        showAccountUsage={isAccountLoginUiEnabled()}
