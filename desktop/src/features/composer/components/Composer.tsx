...
import { attachmentsEnabledForModel } from "../../models/utils/modelCapabilities";
...
type ComposerProps = {
  ...
  models: { id: string; displayName: string; model: string; inputModalities?: string[] }[];
...
export const Composer = memo(function Composer({
  ...
  const selectedModelForAttachments =
    models.find((model) => model.id === selectedModelId) ?? null;
  const attachmentsEnabled = attachmentsEnabledForModel(
    selectedModelForAttachments,
    visionFallbackReady,
  );
  const canSend =
    text.trim().length > 0 || attachedImages.length > 0 || attachedFiles.length > 0;
  ...
  const handleSend = useCallback((submitIntent: ComposerSendIntent = "default") => {
    ...
    if (resolvedMentions.length > 0) {
      onSend(trimmed, attachedImages, attachedFiles, resolvedMentions, submitIntent);
    } else {
      onSend(trimmed, attachedImages, attachedFiles, undefined, submitIntent);
    }
    resetHistoryNavigation();
    setComposerText("");
  ...
  return (
    <footer className={`composer${disabled ? " is-disabled" : ""}`}>
      ...
      {isProcessing && composerFollowUpHintEnabled && (
        <div className="composer-followup-hint" role="status" aria-live="polite">
          <div className="composer-followup-title">
            {t("composer.followUp.title", "Follow-up behavior")}
          </div>
          <div className="composer-followup-copy">
            {oppositeFallsBackToQueue ? (
              t(
                "composer.followUp.defaultQueueUnavailable",
                "Default: Queue (Steer unavailable). Both Enter and {shortcut} will queue this message.",
              ).replace("{shortcut}", followUpShortcutLabel)
            ) : (
              t(
                "composer.followUp.defaultMode",
                "Default: {mode}. Press {shortcut} to {action} this message.",
              )
                .replace(
                  "{mode}",
                  effectiveFollowUpBehavior === "steer"
                    ? t("composer.followUp.mode.steer", "Steer")
                    : t("composer.followUp.mode.queue", "Queue"),
                )
                .replace("{shortcut}", followUpShortcutLabel)
                .replace(
                  "{action}",
                  oppositeFollowUpIntent === "steer"
                    ? t("composer.followUp.action.steer", "steer")
                    : t("composer.followUp.action.queue", "queue"),
                )
            )}
          </div>
        </div>
      ...
      <ComposerInput
        ...
        onAddAttachment={onPickAttachments}
