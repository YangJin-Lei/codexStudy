...
describe("Composer attachments integration", () => {
  ...
  it("keeps image attachment affordances available for text-only models", async () => {
    const restoreFileReader = setMockFileReader();
    const harness = renderComposerHarness({
      activeThreadId: "thread-1",
      activeWorkspaceId: "ws-1",
    ...
    const textarea = getTextarea(harness.container);
    const attachButton = harness.container.querySelector(".composer-attach");
    if (!(attachButton instanceof HTMLButtonElement)) {
      throw new Error("Attach button missing");
    }

    expect(attachButton.disabled).toBe(false);
    expect(attachButton.title).toContain("vision fallback");

    const droppedImage = new File(["data"], "blocked.png", { type: "image/png" });
    (droppedImage as File & { path?: string }).path = "/tmp/blocked.png";

    await act(async () => {
      dispatchDrop(textarea, [droppedImage]);
    });
    expect(getAttachmentNames(harness.container)).toEqual(["blocked.png"]);

    const pastedImage = new File(["data"], "pasted.png", { type: "image/png" });
    const imageItem = { type: "image/png", getAsFile: () => pastedImage };

    await act(async () => {
      dispatchPaste(textarea, [imageItem]);
    });
    expect(getAttachmentNames(harness.container)).toEqual([
      "blocked.png",
      "pasted.png",
