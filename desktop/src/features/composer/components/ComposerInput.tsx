...
export function ComposerInput({
  ...
  const attachmentLabel = t("composer.addAttachment", "Add attachment");
  const attachmentDisabledTitle =
    resolvedLanguage === "zh-CN"
      ? "添加附件（纯文本模型会使用视觉回退或显示设置提示）"
      : "Add attachment (text-only models will use vision fallback or show a setup warning)";
  const disabledPlaceholder =
    resolvedLanguage === "zh-CN"
      ? "Review 正在进行，完成后会重新启用聊天。"
      : "Review in progress. Chat will re-enable when it completes.";
  const defaultPlaceholder =
    resolvedLanguage === "zh-CN"
      ? "告诉 CodexStudy 你想做什么..."
      : "Ask CodexStudy to do something...";
      ...
      >
        ...
        <div className="composer-input-row">
          <button
            ...
            title={attachmentsEnabled ? attachmentLabel : attachmentDisabledTitle}
          ...
          <textarea
            ref={textareaRef}
            placeholder={
              disabled ? disabledPlaceholder : defaultPlaceholder
            }
            value={text}
