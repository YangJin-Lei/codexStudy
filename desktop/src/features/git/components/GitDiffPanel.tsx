...
type GitDiffPanelProps = {
  ...
  commitsAhead?: number;
  codexNewSecurityEnabled?: boolean;
  codexNewSecurityToggleDisabled?: boolean;
...
export function GitDiffPanel({
  ...
  syncError = null,
  commitsAhead = 0,
  codexNewSecurityEnabled = false,
  codexNewSecurityToggleDisabled = false,
  codexNewActiveWorkspace = null,
  ...
  return (
    <PanelShell
      ...
      headerRight={
        <div className="git-panel-actions" role="group" aria-label="Git panel">
          <div className="codex-new-toolbar-group" role="group" aria-label="codex-new">
            <button
              type="button"
            ...
            <button
              type="button"
              className={`codex-new-toolbar-button is-icon-only${
                codexNewSecurityEnabled ? " is-active" : ""
              }${codexNewSecurityToggleDisabled ? " is-disabled" : ""}`}
              onClick={() => void onToggleCodexNewSecurity?.()}
              aria-label={t("codexNew.security", "Security")}
              aria-pressed={codexNewSecurityEnabled}
              title={
                codexNewSecurityToggleDisabled
                  ? t(
                      "codexNew.securityComputerUseDisabled",
                      "Safe mode is unavailable in Computer Use",
                    )
                  : t("codexNew.security", "Security")
              }
              disabled={!workspaceId || !workspacePath || codexNewSecurityToggleDisabled}
            >
    ...
    >
      <CodexNewStatusCard
        ...
        securityEnabled={codexNewSecurityEnabled}
        securityToggleDisabled={codexNewSecurityToggleDisabled}
        activeWorkspace={codexNewActiveWorkspace}
