...
export function usePullRequestComposer({
  ...
  const handleComposerSend = useCallback<ComposerSendHandler>(
    async (text, images = [], files = [], appMentions, submitIntent) => {
      if (isPullRequestComposer) {
        await handleSendPullRequestQuestion(
          text,
          images,
          files,
          appMentions,
      ...
      await handleSend(text, images, files, appMentions, submitIntent);
    },
    [handleSend, handleSendPullRequestQuestion, isPullRequestComposer],
  );
