...
vi.mock("@/codexStudyUiFlags", () => ({
  SHOW_ACCOUNT_LOGIN_UI: true,
  isAccountLoginUiEnabled: () => true,
}));
...
describe("Home", () => {
  ...
  it("renders expanded token stats and account limits", () => {
    ...
    const currentWeekBar = Array.from(
      document.querySelectorAll<HTMLElement>(".home-usage-bar"),
    ).find((bar) => bar.getAttribute("data-value")?.endsWith("36 tokens"));
    expect(currentWeekBar?.getAttribute("data-value")).toContain("36 tokens");
