...
export function HomeUsageSection({
  ...
  const translate = useMemo(
    () => (key: string, fallback: string) =>
      t(key as Parameters<typeof t>[0], fallback),
    [t],
  );
  ...
  const chartRangeAriaLabel =
    chartDays.length > 0
      ? `${t("home.usage.chartWeek", "Usage week")} ${chartDays[0]?.day} ${t("home.usage.to", "to")} ${chartDays[chartDays.length - 1]?.day}`
      : t("home.usage.chartWeek", "Usage week");
  ...
  return (
    <div className="home-usage">
      <div className="home-section-header">
        <div className="home-section-title">{t("home.usageSnapshot", "Usage snapshot")}</div>
        <div className="home-section-meta-row">
          ...
          <button
            ...
            aria-label={t("home.refreshUsage", "Refresh usage")}
            title={t("home.refreshUsage", "Refresh usage")}
          >
            <RefreshCw
              className={
                isLoadingLocalUsage
      ...
      <div className="home-usage-controls">
        <div className="home-usage-control-group">
          <span className="home-usage-control-label">
            {t("home.usage.workspace", "Workspace")}
          </span>
          <div className="home-usage-select-wrap">
            ...
            >
              <option value="">{t("home.usage.allWorkspaces", "All workspaces")}</option>
        ...
        <div className="home-usage-control-group">
          <span className="home-usage-control-label">{t("home.usage.view", "View")}</span>
          <div className="home-usage-toggle" role="group" aria-label={t("home.usage.view", "View")}>
            <button
              type="button"
              className={
                usageMetric === "tokens"
      ...
      {showUsageSkeleton ? (
        ...
        <div className="home-usage-empty">
          <div className="home-usage-empty-title">
            {t("home.usage.noDataTitle", "No usage data yet")}
          </div>
          <div className="home-usage-empty-subtitle">
            {t(
              "home.usage.noDataSubtitle",
        ...
        <>
          ...
          <div className="home-usage-chart-card">
              ...
              <div className="home-usage-chart-actions">
                {canShowOlderWeek && (
                  <button
                    ...
                    aria-label={t("home.usage.showPreviousWeek", "Show previous week")}
                    title={t("home.usage.showPreviousWeek", "Show previous week")}
                  >
                ...
                <button
                  ...
                  aria-label={t("home.usage.showNextWeek", "Show next week")}
                  title={t("home.usage.showNextWeek", "Show next week")}
            ...
            <div className="home-usage-chart">
              {chartDays.map((day) => {
                ...
                const tooltip =
                  usageMetric === "tokens"
                    ? `${formatDayLabel(day.day)} · ${formatCount(day.totalTokens)} ${t("home.usage.suffix.tokens", "tokens")}`
                    : `${formatDayLabel(day.day)} · ${formatDuration(day.agentTimeMs ?? 0)} ${t("home.usage.suffix.agentTime", "agent time")}`;
                return (
          ...
          <div className="home-usage-models">
            <div className="home-usage-models-label">
              {t("home.usage.topModels", "Top models")}
              {usageMetric === "time" && (
                <span className="home-usage-models-hint">
                  {t("home.usage.suffix.tokens", "Tokens")}
                </span>
              )}
            </div>
              ...
              {localUsageSnapshot?.topModels?.length ? (
                localUsageSnapshot.topModels.map((model) => (
                  <span
                    ...
                    key={model.model}
                    title={`${model.model}: ${formatCount(model.tokens)} ${t("home.usage.suffix.tokens", "tokens")}`}
                  >
                    {model.model}
                ...
                <span className="home-usage-model-empty">
                  {t("home.usage.noModels", "No models yet")}
                </span>
              )}
            </div>
      ...
      {isAccountLoginUiEnabled() && accountCards.length > 0 && (
        <div className="home-account">
          <div className="home-section-header">
            <div className="home-section-title">{t("home.accountLimits", "Account limits")}</div>
            {accountMeta && (
              <div className="home-section-meta-row">
