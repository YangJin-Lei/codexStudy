...
describe("useResizablePanels", () => {
  ...
  it("reads stored sizes and clamps to bounds", () => {
    window.localStorage.setItem("codexstudy.sidebarWidth", "999");
    window.localStorage.setItem("codexstudy.rightPanelWidth", "100");
    window.localStorage.setItem("codexstudy.planPanelHeight", "not-a-number");
  ...
  it("persists sidebar width changes and clamps max", () => {
    ...
    expect(hook.result.sidebarWidth).toBe(420);
    expect(window.localStorage.getItem("codexstudy.sidebarWidth")).toBe(
      "420",
    );
