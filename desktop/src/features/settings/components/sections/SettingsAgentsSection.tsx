...
export function SettingsAgentsSection({
  ...
  const handleOpenPath = async (path: string) => {
    ...
    try {
      ...
      setOpenPathError(
        openError instanceof Error ? openError.message : t("settings.agents.openPathError", "Unable to open path."),
      );
    }
  };
  ...
  const handleMaxThreadsChange = async (rawValue: string) => {
    ...
    if (parsed == null) {
      ...
      setEditError(null);
      setOpenPathError(
        t("settings.agents.maxThreads.error", `Max threads must be an integer between ${MIN_MAX_THREADS} and ${MAX_MAX_THREADS}.`),
      );
      return;
    }
    setOpenPathError(null);
  ...
  const handleMaxDepthChange = async (rawValue: string) => {
    ...
    if (parsed == null) {
      ...
      setEditError(null);
      setOpenPathError(
        t("settings.agents.maxDepth.error", `Max depth must be an integer between ${MIN_MAX_DEPTH} and ${MAX_MAX_DEPTH}.`),
      );
      return;
    }
    setOpenPathError(null);
  ...
  const handleCreateAgent = async () => {
    ...
    if (!name) {
      setCreateError(t("settings.agents.nameRequired", "Agent name is required."));
      return;
    }
    setCreateError(null);
  ...
  const handleUpdateAgent = async () => {
    ...
    if (!nextName) {
      setEditError(t("settings.agents.nameRequired", "Agent name is required."));
      return;
    }
  ...
  return (
    <SettingsSection
      title={t("settings.agents.title", "Agents")}
      subtitle={t("settings.agents.subtitle", "Configure multi-agent mode, limits, and custom agent roles.")}
    >
      <div className="settings-help settings-agents-builtins-help">
        {t("settings.agents.builtins", "Built-in roles from Codex are still available:")} <code>default</code>, <code>explorer</code>, {t("settings.agents.builtinsSuffix", "and")} <code>worker</code>.
      </div>

      <SettingsToggleRow
        title={t("settings.features.config.title", "Config file")}
        subtitle={t(
          "settings.agents.config.subtitle",
      ...
      <SettingsToggleRow
        title={t("settings.agents.enableMultiAgent", "Enable Multi-Agent")}
        subtitle={t(
          "settings.agents.enableMultiAgent.subtitle",
      ...
      <SettingsToggleRow
        title={t("settings.agents.maxThreads.title", "Max Threads")}
        subtitle={t(
          "settings.agents.maxThreads.subtitle",
          "Maximum open agent threads. Valid range: 1-12. Changes save immediately.",
        )}
      >
        <div className="settings-agents-stepper" role="group" aria-label={t("settings.agents.maxThreads.aria", "Maximum agent threads")}>
          <button
            type="button"
            ...
            aria-label={t("settings.agents.maxThreads.decrease", "Decrease max threads")}
          >
            ▼
          </button>
          ...
          <button
            ...
            aria-label={t("settings.agents.maxThreads.increase", "Increase max threads")}
          >
            ▲
          </button>
        </div>
      ...
      <SettingsToggleRow
        title={t("settings.agents.maxDepth.title", "Max Depth")}
        subtitle={t(
          "settings.agents.maxDepth.subtitle",
          "Maximum nested spawn depth. Valid range: 1-4. Changes save immediately.",
        )}
      >
        <div className="settings-agents-stepper" role="group" aria-label={t("settings.agents.maxDepth.aria", "Maximum agent depth")}>
          <button
            type="button"
            className="ghost settings-agents-stepper-button"
            onClick={() => {
            ...
            aria-label={t("settings.agents.maxDepth.decrease", "Decrease max depth")}
          >
            ▼
          </button>
          <div className="settings-agents-stepper-value" aria-live="polite" aria-atomic="true">
          ...
          <button
            ...
            aria-label={t("settings.agents.maxDepth.increase", "Increase max depth")}
          >
            ▲
          </button>
        </div>
      ...
      <SettingsSubsection
        title={t("settings.agents.create.title", "Create Agent")}
        subtitle={t(
          "settings.agents.create.subtitle",
      ...
      <div className="settings-field settings-agents-form">
        <div className="settings-agents-description-row">
          ...
          <button
            ...
            title={t("settings.agents.generate", "Generate description and developer instructions with AI")}
            aria-label={t("settings.agents.generateNew", "Generate fields for new agent")}
          >
            {createDescriptionGenerating ? (
        ...
        <input
          ...
          placeholder={t("settings.agents.placeholder.name", "researcher")}
          disabled={creatingAgent}
        />
        <label className="settings-label" htmlFor="settings-agent-create-description">
          {t("settings.common.description", "Description")}
        </label>
        <textarea
          ...
          placeholder={t("settings.agents.placeholder.description", "Short role summary.")}
          rows={2}
          disabled={creatingAgent}
        />
        <label className="settings-label" htmlFor="settings-agent-create-developer-instructions">
          {t("settings.agents.developerInstructions", "Developer instructions")}
        </label>
        <textarea
          ...
          placeholder={t("settings.agents.placeholder.instructions", "Multiline per-agent developer instructions.")}
          disabled={creatingAgent}
        />
        <div className="settings-agents-model-row">
          <div className="settings-agents-model-field settings-agents-model-field--model">
            <span className="settings-agents-inline-label">{t("settings.agents.model", "model")}:</span>
            <select
              ...
              disabled={creatingAgent}
              aria-label={t("settings.agents.modelAria", "Agent model")}
            >
          ...
          <div className="settings-agents-model-field settings-agents-model-field--effort">
            <span className="settings-agents-inline-label">{t("settings.agents.reasoning", "reasoning")}:</span>
            <select
              ...
              aria-label={t("settings.agents.reasoningAria", "Agent reasoning effort")}
            >
              {createReasoningOptions.length === 0 && (
                <option value="">{t("settings.common.notSupported", "not supported")}</option>
              )}
        ...
        <div className="settings-agents-actions">
          <button type="button" className="ghost" onClick={() => void handleCreateAgent()}>
            {creatingAgent
              ? t("settings.agents.creating", "Creating...")
              : t("settings.agents.create.title", "Create Agent")}
          </button>
        </div>
      ...
      <SettingsSubsection
        title={t("settings.agents.configured.title", "Configured Agents")}
        subtitle={t("settings.agents.configured.subtitle", "Manage custom roles and their per-agent config files.")}
      />

      {settings && settings.agents.length === 0 && !isLoading && (
        <div className="settings-help">{t("settings.agents.empty", "No custom agents configured yet.")}</div>
      )}
        ...
        return (
          <div className="settings-field settings-agent-card" key={agent.name}>
            <div className="settings-agent-card-header">
              <div>
                ...
                <div className="settings-toggle-subtitle">
                  {agent.description || t("settings.agents.noDescription", "No description.")}
                </div>
              </div>
            </div>
            ...
            <div className="settings-help settings-help-inline">
              <code>
                {agent.configFile ||
                  t("settings.agents.missingConfigFile", "(missing config_file)")}
              </code>
            </div>
            <div className="settings-agents-actions">
              {!isPendingDelete && (
              ...
              >
                {isReadingConfig ? t("settings.agents.opening", "Opening...") : t("settings.agents.editFile", "Edit File")}
              </button>
              {!agent.managedByApp && (
                <span className="settings-help settings-help-inline">{t("settings.agents.externalPath", "External path")}</span>
              )}
            </div>
            {isPendingDelete && (
              <div className="settings-agents-actions">
                <span className="settings-help settings-help-inline">
                  {t("settings.agents.deleteConfirm", "Delete agent and managed config file?")}
                </span>
                <button
                  type="button"
                  className="ghost"
                ...
                <button
                  ...
                  disabled={isDeleting}
                >
                  {isDeleting ? t("settings.agents.deleting", "Deleting...") : t("settings.agents.confirmDelete", "Confirm Delete")}
                </button>
              </div>
            )}
              ...
              <div className="settings-agents-edit-form">
                <div className="settings-agents-description-row">
                  ...
                  <button
                    ...
                    title={t("settings.agents.generate", "Generate description and developer instructions with AI")}
                    aria-label={t(
                      "settings.agents.generateExisting",
                      "Generate fields for this agent",
                    )}
                  >
                    {editDescriptionGenerating ? (
                      <MagicSparkleLoaderIcon className="settings-agents-generate-loader" />
                ...
                <label className="settings-checkbox">
                  ...
                  />
                  {t("settings.agents.renameManagedFile", "Rename managed config file when agent name changes")}
                </label>
                <div className="settings-agents-actions">
                  <button
                    type="button"
                  ...
                  <button
                    ...
                    disabled={isUpdating}
                  >
                    {isUpdating ? t("settings.common.saving", "Saving...") : t("settings.common.save", "Save")}
                  </button>
                </div>
                ...
                <div className="settings-agents-header">
                  <div>
                    <div className="settings-toggle-title">
                      {agent.name} {t("settings.agents.configFile", "config file")}
                    </div>
                    <div className="settings-toggle-subtitle">
                  ...
                  <div className="settings-agents-actions">
                    ...
                    >
                      {isWritingConfig ? t("settings.common.saving", "Saving...") : t("settings.common.save", "Save")}
                    </button>
                  </div>
                </div>
                <textarea
      ...
      {isLoading && <div className="settings-help">{t("settings.agents.loading", "Loading agents settings...")}</div>}
