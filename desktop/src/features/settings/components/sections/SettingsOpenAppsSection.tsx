...
export function SettingsOpenAppsSection({
  ...
  return (
    <SettingsSection
      title={t("settings.openApps.title", "Open in")}
      subtitle={t("settings.openApps.subtitle", "Customize the Open in menu shown in the title bar and file previews.")}
    >
      <div className="settings-open-apps">
        {openAppDrafts.map((target, index) => {
          ...
          const incompleteHint = !labelValid
            ? t("settings.openApps.required.label", "Label required")
            : target.kind === "app"
              ? t("settings.openApps.required.appName", "App name required")
              : target.kind === "command"
                ? t("settings.openApps.required.command", "Command required")
                : t("settings.openApps.required.complete", "Complete required fields");
            ...
            >
              ...
              <div className="settings-open-app-fields">
                <label className="settings-open-app-field settings-open-app-field--label">
                  <span className="settings-visually-hidden">{t("settings.openApps.fields.label", "Label")}</span>
                  <input
                    className="settings-input settings-input--compact settings-open-app-input settings-open-app-input--label"
                    value={target.label}
                    placeholder={t("settings.openApps.fields.label", "Label")}
                    onChange={(event) =>
                ...
                <label className="settings-open-app-field settings-open-app-field--type">
                  <span className="settings-visually-hidden">{t("settings.openApps.fields.type", "Type")}</span>
                  <select
                  ...
                  >
                    <option value="app">{t("settings.openApps.kind.app", "App")}</option>
                    <option value="command">{t("settings.openApps.kind.command", "Command")}</option>
                    <option value="finder">{fileManagerName()}</option>
                  </select>
                </label>
                {target.kind === "app" && (
                  <label className="settings-open-app-field settings-open-app-field--appname">
                    <span className="settings-visually-hidden">{t("settings.openApps.fields.appName", "App name")}</span>
                    <input
                      ...
                      placeholder={t("settings.openApps.fields.appName", "App name")}
                      onChange={(event) =>
                        onOpenAppDraftChange(index, {
                ...
                {target.kind === "command" && (
                  <label className="settings-open-app-field settings-open-app-field--command">
                    <span className="settings-visually-hidden">{t("settings.openApps.fields.command", "Command")}</span>
                    <input
                      ...
                      placeholder={t("settings.openApps.fields.command", "Command")}
                      onChange={(event) =>
                        onOpenAppDraftChange(index, {
                ...
                {target.kind !== "finder" && (
                  <label className="settings-open-app-field settings-open-app-field--args">
                    <span className="settings-visually-hidden">{t("settings.openApps.fields.args", "Args")}</span>
                    <input
                      ...
                      placeholder={t("settings.openApps.fields.args", "Args")}
                      onChange={(event) =>
                        onOpenAppDraftChange(index, {
              ...
              <div className="settings-open-app-actions">
                {!isComplete && (
                  ...
                  >
                    {t("settings.openApps.incomplete", "Incomplete")}
                  </span>
                )}
                <label className="settings-open-app-default">
                  <input
                    ...
                    disabled={!isComplete}
                  />
                  {t("settings.common.default", "Default")}
                </label>
                <div className="settings-open-app-order">
                  <button
                    ...
                    disabled={index === 0}
                    aria-label={t("settings.common.moveUp", "Move up")}
                  >
                  ...
                  <button
                    ...
                    aria-label={t("settings.common.moveDown", "Move down")}
                  >
                ...
                <button
                  ...
                  aria-label={t("settings.openApps.removeApp", "Remove app")}
                  title={t("settings.openApps.removeApp", "Remove app")}
                >
                  <Trash2 aria-hidden />
                </button>
              </div>
      ...
      <div className="settings-open-app-footer">
        <button type="button" className="ghost" onClick={onAddOpenApp}>
          {t("settings.openApps.addApp", "Add app")}
        </button>
        <div className="settings-help">
          {t("settings.openApps.help.commandArg", "Commands receive the selected path as the final argument.")}{" "}
          {isMacPlatform()
            ? t("settings.openApps.help.mac", "Apps open via `open -a` with optional args.")
            : t("settings.openApps.help.other", "Apps run as an executable with optional args.")}
        </div>
      </div>
    </SettingsSection>
  );
