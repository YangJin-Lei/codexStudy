...
import {
  ...
  getCodexNewState as getCodexNewStateService,
...
import { readCodexNewState } from "@/features/codex-new/state";

function securityPathAliasesFromState(
  workspaceSecurity: ReturnType<typeof readCodexNewState>["workspaceSecurity"],
) {
  return Object.fromEntries(
    Object.entries(workspaceSecurity).map(([workspaceId, entry]) => [
      workspaceId,
      entry.pathAliases ?? [],
    ]),
  );
...
type CodexNewThreadContext = {
  pathAliases: Record<string, string[]>;
  threadRegistry: Record<string, { workspaceId: string }>;
};

async function loadCodexNewThreadContext(): Promise<CodexNewThreadContext> {
  try {
    const remote = await getCodexNewStateService();
    return {
      pathAliases: securityPathAliasesFromState(remote.workspaceSecurity),
      threadRegistry: Object.fromEntries(
        Object.entries(remote.threadRegistry ?? {}).map(([threadId, entry]) => [
          threadId,
    ...
    const local = readCodexNewState();
    return {
      pathAliases: securityPathAliasesFromState(local.workspaceSecurity),
      threadRegistry: Object.fromEntries(
        Object.entries(local.threadRegistry ?? {}).map(([threadId, entry]) => [
          threadId,
          { workspaceId: entry.workspaceId },
        ]),
      ),
...
const THREAD_LIST_CURSOR_PAGE_START = "__codexstudy_page_start__";
...
export function useThreadActions({
  ...
  const startThreadForWorkspace = useCallback(
    async (workspaceId: string, options?: { activate?: boolean }) => {
      ...
      try {
        const response = await startThreadService(workspaceId, { modelDisplayName });
        onDebug?.({
  ...
  const resumeThreadForWorkspace = useCallback(
    async (
      ...
      try {
        const response =
          (await resumeThreadService(workspaceId, threadId, { modelDisplayName })) as
  ...
  const listThreadsForWorkspaces = useCallback(
    async (
      ...
      try {
        ...
        const codexNewContext = await loadCodexNewThreadContext();
        let workspacePathLookup = buildWorkspacePathLookup(
          targets,
          codexNewContext.pathAliases,
        );
        ...
        do {
          ...
          data.forEach((thread) => {
            const workspaceId = resolveWorkspaceIdForThread(
              thread,
              workspacePathLookup,
              codexNewContext.threadRegistry,
              targetWorkspaceIds,
            );
            if (!workspaceId) {
              return;
