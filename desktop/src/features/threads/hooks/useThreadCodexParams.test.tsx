...
describe("useThreadCodexParams", () => {
  ...
  it("clears cached model selections and syncs in the same window", async () => {
    const { result } = renderHook(() => useThreadCodexParams());

    act(() => {
      result.current.patchThreadCodexParams("ws-1", "thread-5", {
        modelId: "gpt-5.5",
        effort: "medium",
        serviceTier: "fast",
        accessMode: "current",
    ...
    await waitFor(() => {
      expect(result.current.getThreadCodexParams("ws-1", "thread-5")).toEqual(
        expect.objectContaining({
          modelId: null,
          effort: null,
          serviceTier: "fast",
