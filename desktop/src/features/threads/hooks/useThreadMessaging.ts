...
export function useThreadMessaging({
  ...
  const sendMessageToThread = useCallback(
    async (
      ...
      text: string,
      images: string[] = [],
      filesOrOptions: string[] | SendMessageOptions = [],
      maybeOptions?: SendMessageOptions,
    ): Promise<SendMessageResult> => {
      const files = Array.isArray(filesOrOptions) ? filesOrOptions : [];
      const options = Array.isArray(filesOrOptions)
        ? maybeOptions
        : filesOrOptions;
      ...
      try {
        if (readCodexNewState().workspaceSecurity[workspace.id]) {
          try {
            ...
            console.warn("Failed to prepare codex-new isolated runtime before send.", {
              error,
            });
          }
        }
  ...
  const sendUserMessage = useCallback(
    async (
      ...
      appMentionsOrOptions:
        | AppMention[]
        | { sendIntent?: ComposerSendIntent }
        | undefined = [],
      maybeOptions?: { sendIntent?: ComposerSendIntent },
    ): Promise<SendMessageResult> => {
      const files =
        filesOrAppMentions.length > 0 && typeof filesOrAppMentions[0] !== "string"
          ? []
          : (filesOrAppMentions as string[]);
      const appMentions =
        filesOrAppMentions.length > 0 && typeof filesOrAppMentions[0] !== "string"
          ? (filesOrAppMentions as AppMention[])
          : Array.isArray(appMentionsOrOptions)
            ? appMentionsOrOptions
            : [];
      const options = Array.isArray(appMentionsOrOptions)
        ? maybeOptions
        : appMentionsOrOptions;
      if (!activeWorkspace) {
        return { status: "blocked" };
      ...
      return sendMessageToThread(activeWorkspace, threadId, finalText, images, files, {
