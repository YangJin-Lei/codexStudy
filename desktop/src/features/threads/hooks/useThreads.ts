...
import { MODEL_PROVIDER_SESSION_REFRESHED_EVENT } from "@settings/modelProviderEvents";
...
export function useThreads({
  ...
  useEffect(() => {
    ...
    const handleProviderSessionRefresh = () => {
      if (!activeWorkspace?.id) {
        return;
      }
      void (async () => {
        resetWorkspaceThreads(activeWorkspace.id);
        const recoveredThreadId = activeThreadId
          ? await refreshThread(activeWorkspace.id, activeThreadId)
          : null;
        if (activeThreadId && !recoveredThreadId) {
          await startThreadForWorkspace(activeWorkspace.id, { activate: true });
        }
        await listThreadsForWorkspace(activeWorkspace, { preserveState: false });
        await refreshAccountRateLimits(activeWorkspace.id);
        await refreshAccountInfo(activeWorkspace.id);
      })();
    };
    window.addEventListener(
      MODEL_PROVIDER_SESSION_REFRESHED_EVENT,
      handleProviderSessionRefresh,
    );
    return () => {
