...
function registerWorkspacePathAlias(
  workspaceId: string,
  rawPath: string,
  workspaceIdsByPath: Record<string, string[]>,
  workspacePathsSorted: string[],
) {
  const workspacePath = normalizeRootPath(rawPath);
  if (!workspacePath) {
    return;
  }
  ...
  if (!workspaceIdsByPath[workspacePath].includes(workspaceId)) {
    workspaceIdsByPath[workspacePath].push(workspaceId);
  }
}

export function buildWorkspacePathLookup(
  workspaces: WorkspaceInfo[],
  securityPathAliases?: Record<string, string[]>,
): WorkspacePathLookup {
  const workspaceIdsByPath: Record<string, string[]> = {};
  const workspacePathsSorted: string[] = [];
  workspaces.forEach((workspace) => {
    registerWorkspacePathAlias(
      workspace.id,
      workspace.path,
      workspaceIdsByPath,
      workspacePathsSorted,
    );
  });
  if (securityPathAliases) {
    Object.entries(securityPathAliases).forEach(([workspaceId, aliases]) => {
...
export function resolveWorkspaceIdForThread(
  ...
  if (threadId) {
    const binding = threadRegistry[threadId];
    if (binding?.workspaceId) {
      if (!allowedWorkspaceIds || allowedWorkspaceIds.has(binding.workspaceId)) {
        return binding.workspaceId;
      }
    }
  }
  const cwd = typeof thread.cwd === "string" ? thread.cwd : "";
  return resolveWorkspaceIdForThreadPath(cwd, lookup, allowedWorkspaceIds);
}
