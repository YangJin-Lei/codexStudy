...
const STORAGE_KEY_THREAD_ACTIVITY = "codexstudy.threadLastUserActivity";
export const STORAGE_KEY_PINNED_THREADS = "codexstudy.pinnedThreads";
export const STORAGE_KEY_CUSTOM_NAMES = "codexstudy.threadCustomNames";
export const STORAGE_KEY_THREAD_CODEX_PARAMS = "codexstudy.threadCodexParams";
export const STORAGE_KEY_DETACHED_REVIEW_LINKS = "codexstudy.detachedReviewLinks";
export const THREAD_CODEX_PARAMS_UPDATED_EVENT = "codexstudy:threadCodexParamsUpdated";
...
export function saveThreadCodexParams(next: ThreadCodexParamsMap): void {
  ...
  try {
    ...
    window.dispatchEvent(new CustomEvent(THREAD_CODEX_PARAMS_UPDATED_EVENT));
  } catch {
    // Best-effort persistence.
  }
}

export function clearThreadCodexModelSelections(): void {
  if (typeof window === "undefined") {
    return;
  }
  try {
    const current = loadThreadCodexParams();
