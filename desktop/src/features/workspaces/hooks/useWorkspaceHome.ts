...
export function useWorkspaceHome({
  ...
  const startRun = useCallback(async (images: string[] = [], files: string[] = []) => {
    ...
    if ((!prompt && !hasImages && !hasFiles) || isSubmitting) {
      return false;
    }
    ...
    if (runMode === "worktree" && selectedModels.length === 0) {
      setWorkspaceError(messages["en-US"]["workspace.modelRequired"]);
      return false;
    }
      ...
      if (runMode === "local") {
        try {
          ...
          const localModel = selectedModelId
            ...
            : null;
          await sendUserMessageToThread(activeWorkspace, threadId, prompt, images, files, {
            model: localModel,
            effort,
            serviceTier,
            collaborationMode,
