...
export function subscribeCodexNewFocusThread(
  onEvent: (payload: CodexNewFocusThreadPayload) => void,
  options?: SubscriptionOptions,
): Unsubscribe {
...
const CODEX_HOME_MIGRATION_OPEN_EVENT = "codex-home-migration-open";

export function requestCodexHomeMigrationPrompt(): void {
  if (typeof window === "undefined") {
    return;
  }
  window.dispatchEvent(new CustomEvent(CODEX_HOME_MIGRATION_OPEN_EVENT));
}

export function subscribeCodexHomeMigrationPrompt(
  onEvent: () => void,
): Unsubscribe {
  if (typeof window === "undefined") {
    return () => {};
  ...
  window.addEventListener(CODEX_HOME_MIGRATION_OPEN_EVENT, handler);
  return () => window.removeEventListener(CODEX_HOME_MIGRATION_OPEN_EVENT, handler);
}
