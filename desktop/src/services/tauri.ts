...
import type {
  ...
  SaveModelProviderSettingsResult,
...
import type {
  ...
  CodexNewMemoryApplyOutcome,
  CodexNewTracebackEntry,
  CodexNewTracebackRestoreTarget,
} from "../features/codex-new/types";
import type {
  GitFileDiff,
  GitFileStatus,
  GitCommitDiff,
...
export async function syncCodexNewViewingContextBackend(
  workspaceId: string,
  threadId: string | null,
  threadTitle: string | null = null,
): Promise<CodexNewFrontendState> {
  return invoke<CodexNewFrontendState>("codex_new_sync_viewing_context", {
    input: { workspaceId, threadId, threadTitle },
  });
}

export type CodexHomeMigrationLegacyHome = {
  path: string;
  sessionFileCount: number;
};

export type CodexHomeMigrationStatus = {
...
export type CodexHomeMigrationImportResult = {
  copiedFiles: number;
  skippedFiles: number;
  copiedConfig: boolean;
};

export async function getCodexHomeMigrationStatus(): Promise<CodexHomeMigrationStatus> {
  return invoke<CodexHomeMigrationStatus>("codex_home_migration_status");
}

export async function importCodexHomeMigration(
  sourcePath: string,
): Promise<CodexHomeMigrationImportResult> {
  return invoke<CodexHomeMigrationImportResult>("codex_home_migration_import", {
    sourcePath,
  });
}

export async function skipCodexHomeMigration(): Promise<void> {
  return invoke<void>("codex_home_migration_skip");
}

export async function getCodexNewState(): Promise<CodexNewFrontendState> {
  try {
    return await invoke<CodexNewFrontendState>("codex_new_get_state");
  } catch (error) {
    if (isMissingTauriInvokeError(error)) {
      return {
        activeSession: null,
...
export async function enableCodexNewSecurityBackend(
  workspaceId: string,
  threadId: string | null,
  threadTitle: string | null = null,
): Promise<CodexNewFrontendState> {
  return invoke<CodexNewFrontendState>("codex_new_enable_security", {
    input: { workspaceId, threadId, threadTitle },
  });
}

export async function focusCodexNewSessionBackend(
  workspaceId: string,
  threadId: string | null,
  threadTitle: string | null = null,
): Promise<CodexNewFrontendState> {
  return invoke<CodexNewFrontendState>("codex_new_focus_session", {
    input: { workspaceId, threadId, threadTitle },
  });
}

export async function syncCodexNewThreadTitlesBackend(
  workspaceId: string,
  entries: { threadId: string; threadTitle: string | null }[],
): Promise<CodexNewFrontendState> {
  return invoke<CodexNewFrontendState>("codex_new_sync_thread_titles", {
    workspaceId,
    entries,
  });
}

export async function disableCodexNewSecurityBackend(
  workspaceId: string,
): Promise<CodexNewFrontendState> {
  return invoke<CodexNewFrontendState>("codex_new_disable_security", { workspaceId });
}

export async function refreshCodexNewChangesBackend(
  workspaceId: string,
): Promise<CodexNewFrontendState> {
...
export async function runCodexNewReviewBackend(
  workspaceId: string,
): Promise<CodexNewFrontendState> {
  return invoke<CodexNewFrontendState>("codex_new_run_review", {
    input: { workspaceId },
  });
}

export async function mergeCodexNewChangesBackend(
  workspaceId: string,
  options?: {
    paths?: string[];
  ...
  return invoke<CodexNewFrontendState>("codex_new_merge_changes", {
    input: {
      workspaceId,
      paths: options?.paths,
      hunks: options?.hunks,
...
export async function listCodexNewTracebackBackend(
  workspaceId: string,
): Promise<CodexNewTracebackEntry[]> {
  return invoke<CodexNewTracebackEntry[]>("codex_new_list_traceback", {
    input: { workspaceId },
  });
}

export async function restoreCodexNewTracebackBackend(
  workspaceId: string,
  path: string,
  target: CodexNewTracebackRestoreTarget,
): Promise<CodexNewFrontendState> {
  return invoke<CodexNewFrontendState>("codex_new_restore_traceback", {
    input: { workspaceId, path, target },
  });
}

export async function listCodexNewMemoryCandidatesBackend(
  workspaceId: string,
): Promise<CodexNewCandidateMemoryRecord[]> {
  return invoke<CodexNewCandidateMemoryRecord[]>("codex_new_list_memory_candidates", {
    input: { workspaceId },
  });
}

export async function applyCodexNewMemoryCandidatesBackend(
  workspaceId: string,
  candidateIds: string[],
): Promise<CodexNewMemoryApplyOutcome> {
...
export async function rollbackCodexNewTaskBackend(
  ...
  return invoke<CodexNewFrontendState>("codex_new_rollback_task", {
    input: {
      workspaceId,
      paths: options?.paths,
      hunks: options?.hunks,
...
export async function writeCodexNewSummaryBackend(
  workspaceId: string,
  goal?: string,
  result?: string,
): Promise<CodexNewFrontendState> {
  return invoke<CodexNewFrontendState>("codex_new_write_summary", {
    input: { workspaceId, goal, result },
  });
}

export async function runCodexNewTestBackend(
  workspaceId: string,
  command: string,
  options?: {
    useEnvironmentBinding?: boolean;
    retryOf?: string | null;
    title?: string;
  },
): Promise<CodexNewFrontendState> {
  return invoke<CodexNewFrontendState>("codex_new_run_test", {
    input: {
      workspaceId,
      command,
      useEnvironmentBinding: options?.useEnvironmentBinding,
...
export async function startThread(
  workspaceId: string,
  options?: { modelDisplayName?: string | null },
) {
...
export async function setComputerUseEnabled(
  ...
  return invoke<ComputerUseActionResult>("computer_use_set_enabled", { enabled });
}

export async function repairComputerUseInstall(): Promise<ComputerUseActionResult> {
  return invoke<ComputerUseActionResult>("computer_use_repair_install");
}

export async function runComputerUseDoctor(): Promise<string> {
  return invoke<string>("computer_use_run_doctor");
}

export async function prepareComputerUseWorkspaceDir(): Promise<string> {
  return invoke<string>("computer_use_prepare_workspace_dir");
}
...
): Promise<SaveModelProviderSettingsResult> {
  return invoke<SaveModelProviderSettingsResult>("save_model_provider_settings", {
    input,
  });
}

export async function diagnoseModelProviderConnection(): Promise<ModelProviderConnectionDiagnostic> {
  return invoke<ModelProviderConnectionDiagnostic>("diagnose_model_provider_connection");
}

export async function getModelProviderHistory(): Promise<ModelProviderHistoryEntry[]> {
  return invoke<ModelProviderHistoryEntry[]>("get_model_provider_history");
}

export async function syncCurrentModelProviderHistoryModels(
  models: string[],
): Promise<ModelProviderHistoryEntry[]> {
  return invoke<ModelProviderHistoryEntry[]>("sync_current_model_provider_history_models", {
    models,
  });
}

export async function deleteModelProviderHistoryEntry(
  id: string,
): Promise<DeleteModelProviderHistoryEntryResult> {
  return invoke<DeleteModelProviderHistoryEntryResult>("delete_model_provider_history_entry", {
    id,
  });
}
